...
const NavItem: React.FC<NavItemProps> = ({ isExpanded, isActive, organization, onExpand }) => {
    ...
    const routes = [
        {
            label: "Board", icon: <Layout className="h-4 w-4 mr-2" />, href: `/organization/${organization.id}`,
        }, {
    ...
    return <AccordionItem value={organization.id} className="border-none">
        ...
        <AccordionContent className="pt-1 text-neutral-700">
            {routes.map((route) => (
                <Button key={route.href} size={"sm"} onClick={() => onClick(route.href)} className={cn("w-full font-normal justify-start pl-10 ml-1", pathname === route.href && "bg-sky-500/10 text-sky-700")} variant={"ghost"}>{route.icon}{route.label}</Button>
            ))}
        </AccordionContent>
    </AccordionItem>;
};
