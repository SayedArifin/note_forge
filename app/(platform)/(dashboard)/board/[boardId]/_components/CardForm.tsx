...
import { FormTextarea } from "@/components/form/FromTextarena";
...
const CardForm = forwardRef<HTMLTextAreaElement, CardFormProps>(({ listId, enableEditing, disableEditing, isEditing }, ref) => {
    ...
    const onTextarenakeyDown: KeyboardEventHandler<HTMLTextAreaElement> = (e) => {
    ...
    if (isEditing) {
        return (
            <form action={onSubmit} ref={formRef} className="m-1 py-0.5 px-1 space-y-4">
                <FormTextarea id="title" onKeyDown={onTextarenakeyDown} ref={ref} placeholder="Enter Title for this card..." errors={fieldErrors} />
